...
with st.sidebar:
    selected = option_menu('Multiple Disease Prediction', [
        ...
        icons=['','activity', 'heart', 'person','person','person','person','bar-chart-fill','virus'],
        default_index=0)
...
#         jaundice_prediction=[[]]
#         jaundice_prediction = lung_cancer_model.predict([[age,Sex,Total_Bilirubin,Direct_Bilirubin,Alkaline_Phosphotase,Alamine_Aminotransferase,Total_Protiens,Albumin]])
...
if selected == 'Healthcare Chatbot':
    # Define the Gemini (PaLM) API endpoint
    ...
    chat_btn=st.button("ask chat bot")

    # Generate bot response
    if chat_btn:
        with st.spinner("Bot is typing..."):
            bot_response = generate_response(user_input)
        st.markdown(f"<div style='padding: 10px;'>{bot_response}</div>", unsafe_allow_html=True)
